"""
OCR UTILS V19 - ROI TEMPORAL RESTAURADO + ESTRELLA VERDE + TUPUNGATITO

CAMBIOS V19:
5. **NUEVO V19:** Detección grupos píxeles rojos para eventos superpuestos (Lastarria 05:42 + 06:06)

FUNCIONALIDADES CRÍTICAS PRESERVADAS:
✅ ROI temporal: x: 0.8424-0.8635, y: 0.1817-0.4933
✅ Sistema 3 fases: rojos → estrella → negros
✅ Límites por volcán: 11 volcanes en LIMITES_Y_COORDENADAS
✅ Filtro zona estrella verde: mask_grafico[100:, 250:]
✅ import os presente
✅ Tupungatito en configuraciones
...
def detectar_grupos_pixeles_rojos(roi, umbral_area_minima=20):
    ...
    Útil para eventos superpuestos en misma hora (ej: Lastarria 05:42 + 06:06)
    ...
    # Detectar píxeles rojos (mismo criterio que análisis global)
    mask_rojos = (
    ...
    # Encontrar contornos de grupos separados
...
def analizar_puntos_distancia(eventos, img_dist_path, volcan_nombre):
    ...
    - Calcula datos POR EVENTO individual
    ...
    try:
        ...
        # ===== ANÁLISIS GLOBAL (compatibilidad V17) =====
        mask_rojos = (
            (roi[:, :, 0] > 150) &
            (roi[:, :, 1] < 100) &
            (roi[:, :, 2] < 100)
...
def clasificar_confianza(evento, img_dist_path, volcan_nombre):
    ...
    # ===== FASE 1 MEJORADA V19: VALIDAR CON GRUPO INDIVIDUAL =====
    ...
    if grupo_info:
        ...
        if y_absoluto >= y_limite_px:
            ...
            distancia_aprox = ((y_absoluto - y_limite_px) / (335 - y_limite_px)) * limite_km
            
            print(f"   ═════════════════════════════════════════════════════════")
            print(f"   🎯 FASE 1 V19 (grupo individual): Y={y_absoluto} >= {y_limite_px} ✅")
            print(f"      ✅ ALERTA_TERMICA_OCR: Grupo píxeles en Y={y_absoluto} (área={area_grupo} px², dist≈{distancia_aprox:.2f} km)")
    ...
    print(f"   ═════════════════════════════════════════════════════════")
    print(f"   🎯 FASE 1 (ROI temporal): Sin grupo individual detectado")
    ...
    # ===== FASE 3: PÍXELES NEGROS (FALLBACK - V17 PRESERVADO) =====
...
def verificar_evento_no_existe(evento, volcan_nombre, sensor, df_consolidado, df_ocr):
    """
    V17: Verifica si evento YA existe en CSVs (evitar duplicados)
    
    MEJORADO V17: Busca en AMBOS CSVs (consolidado + OCR)
    """
    
    timestamp = evento.get('timestamp')
    if not timestamp:
        return True  # Sin timestamp, considerarlo nuevo
    
    print(f"\n   🔍 VERIFICANDO DUPLICADOS:")
    print(f"      Buscando: ts={timestamp}, volcan={volcan_nombre}, sensor={sensor}")
    
    # Buscar en CSV consolidado
    duplicado_latest = False
    ...
    es_nuevo = not (duplicado_latest or duplicado_ocr)
